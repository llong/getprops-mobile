...
export function useProfileQuery(userId?: string) {
    return useQuery({
        ...
        queryFn: () => userId ? profileService.fetchIdentity(userId) : Promise.resolve(null),
        enabled: !!userId,
...
export function useUserContentQuery(userId?: string, enabled = true) {
    return useQuery({
        ...
        queryFn: () => userId ? profileService.fetchUserContent(userId) : Promise.resolve(null),
        enabled: !!userId && enabled,
        staleTime: 1000 * 60 * 5, // 5 minutes
    });
}
...
export function useNotificationsQuery(userId?: string) {
    return useQuery({
        ...
        queryFn: () => userId ? profileService.fetchNotifications(userId) : Promise.resolve([]),
        enabled: !!userId,
...
export function useUserFollowersQuery(userId?: string) {
    return useInfiniteQuery({
        ...
        queryFn: async ({ pageParam }) => {
            const data = await profileService.fetchUserFollowers(userId!, pageParam as string | null);
            return {
...
export function useUserFollowingQuery(userId?: string) {
    return useInfiniteQuery({
        ...
        queryFn: async ({ pageParam }) => {
            const data = await profileService.fetchUserFollowing(userId!, pageParam as string | null);
            return {
