...
export const feedService = {
  ...
  async fetchGlobalFeed(
    ...
    console.log(
      "[FeedService] Fetching feed. User:",
      userId,
      "Filters:",
    ...
    response = await supabase.rpc("get_global_feed_content", {
      ...
      p_spot_types: (filters?.spotTypes?.length ?? 0) > 0 ? filters?.spotTypes : null,
      p_difficulties: (filters?.difficulties?.length ?? 0) > 0 ? filters?.difficulties : null,
      p_min_risk: filters?.minRisk || null,
      p_max_risk: filters?.maxRisk || null,
      p_rider_types: (filters?.riderTypes?.length ?? 0) > 0 ? filters?.riderTypes : null,
    ...
    // The RPC returns JSONB which comes back as an array containing one object with the function name as key
    let feedItems: FeedItem[] = [];
    if (Array.isArray(data) && data.length > 0) {
      if (data[0].get_global_feed_content) {
        feedItems = data[0].get_global_feed_content;
      } else {
    ...
    if (userId && feedItems.length > 0) {
      const photoIds = feedItems
        .filter((item) => item.media_type === "photo")
        .map((item) => item.media_id);
      const videoIds = feedItems
      ...
      const [likesResult, favoritesResult] = await Promise.all([
        supabase
          ...
          .eq("user_id", userId)
          .or(
            `photo_id.in.(${photoIds.join(",")}),video_id.in.(${videoIds.join(",")})`
          ),
        supabase
      ...
      const likedPhotoIds = new Set(
        likesResult.data?.map((l) => l.photo_id).filter(Boolean)
      );
      const likedVideoIds = new Set(
        likesResult.data?.map((l) => l.video_id).filter(Boolean)
      );
      const favoritedSpotIds = new Set(
        favoritesResult.data?.map((f) => f.spot_id)
      );
  ...
  async fetchFollowingFeed(
    ...
    let feedItems: FeedItem[] = [];
    if (Array.isArray(data) && data.length > 0) {
      if (data[0].get_following_feed_content) {
        feedItems = data[0].get_following_feed_content;
      } else {
        feedItems = data as FeedItem[];
      }
    }

    // Enrichment for following feed
    if (userId && feedItems.length > 0) {
      const photoIds = feedItems
        .filter((item) => item.media_type === "photo")
        .map((item) => item.media_id);
