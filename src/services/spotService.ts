...
export const spotService = {
  ...
  async fetchSpotDetails(spotId: string, userId?: string) {
    ...
    const favoritedUsersPromise = supabase.from("user_favorite_spots").select(`
                user_id,
                profiles (
                    id,
                    username,
            ...
            `);
    // Note: We'll filter the favorite by spot_id in the results to avoid PostgREST join issues if they arise,
    // but better to add .eq('spot_id', spotId) if supported by the join.
    // For mobile parity with web service:
    (favoritedUsersPromise as any).eq("spot_id", spotId);
    ...
    const { data: profiles } =
      authorIds.length > 0
        ? await supabase
            .from("profiles")
            .select('id, username, "avatarUrl", "displayName"')
            .in("id", authorIds)
        : { data: [] };
    ...
    const photos: MediaItem[] = (spotData.spot_photos || []).map((p: any) => {
      ...
      return {
        ...
        isLiked: userId
          ? p.media_likes?.some((l: any) => l.user_id === userId)
          : false,
      };
    });
    ...
    const videos: MediaItem[] = (spotData.spot_videos || []).map((v: any) => {
      ...
      return {
        ...
        isLiked: userId
          ? v.media_likes?.some((l: any) => l.user_id === userId)
          : false,
      };
    });
    ...
    return {
      ...
      isFavorited: !!(
        userId &&
        favoriteStatusResult.data &&
        favoriteStatusResult.data.length > 0
      ),
  ...
  async toggleFavorite(spotId: string, userId: string, isFavorited: boolean) {
    if (isFavorited) {
      ...
      const { error } = await supabase.from("user_favorite_spots").upsert(
        { user_id: userId, spot_id: spotId },
        { onConflict: "user_id,spot_id" } // In mobile, upsert with onConflict might need different syntax depending on version, but standard is this
      );
      if (error) throw error;
    }
  },
