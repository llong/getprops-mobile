...
import type { UserProfile, ChatMessage, Conversation } from "@/types";
...
export const chatService = {
  ...
  async getOrCreate1on1(myId: string, targetUserId: string): Promise<string> {
    ...
    if (error) {
      ...
      const common = myConvs?.filter((m) =>
        targetConvs?.some((t) => t.conversation_id === m.conversation_id)
      );
  ...
  async fetchConversationDetails(conversationId: string): Promise<Conversation> {
    ...
    const participantIds = data.conversation_participants.map(
      (p: any) => p.user_id
    );
    const { data: profiles } = await supabase
      .from("profiles")
